...
eval4 (Program statement var) = do _ <- eval4_helper statement
                                   env <- get
                                   lift $ eval2 env (Var var)
                                   where
                                     eval4_helper :: DietStatement -> Eval4 Value
                                     eval4_helper (DAssign varName expr) = do
                                       env <- get
                                       val <- lift $ eval2 env expr
                                       put $ extend env varName val
                                       return val
                                     eval4_helper (DIf expr statement1 statement2) = do
                                       env <- get
                                       val <- lift $ eval2 env expr
                                       case val of
                                         IntVal res -> if res == 1
                                                       then eval4_helper statement1
                                                       else eval4_helper statement2
                                         _ -> lift $ throwError "Type error in DIf"
                                     eval4_helper (DWhile expr statement') = eval4_helper (DIf expr
                                                                              (DSequence statement'
                                                                                         (DWhile expr statement'))
                                                                              DSkip)
                                     eval4_helper (DSequence statement1 statement2) = do _ <- eval4_helper statement1
                                                                                   eval4_helper statement2
                                     eval4_helper DSkip = return (IntVal 0)


type Eval2_helper a = ExceptT String IO a

eval2_helper :: Env -> Expression -> eval2_helper Value
eval2_helper env (Var name) = case (Map.lookup name env) of
                            Nothing -> throwError "Variable is not in environment"
                            Just x -> return x
eval2_helper _   (Lit i) = return $ IntVal i
eval2_helper env (Op e1 op e2) = do val1 <- eval2_helper env e1
                              val2 <- eval2_helper env e2
                              case (val1, val2) of
                                   (IntVal i1, IntVal i2) -> return $ IntVal $ opFun op i1 i2
                                   _ -> throwError "Type error in Op"
eval2_helper env (Lambda name body) = return $ FunVal env name body
eval2_helper env (Ap e1 e2) = do arg1 <- eval2_helper env e1
                           val2 <- eval2_helper env e2
                           case arg1 of
                                FunVal env' name body -> eval2_helper (extend env' name val2) body
                                _ -> throwError "Type error in Ap"
